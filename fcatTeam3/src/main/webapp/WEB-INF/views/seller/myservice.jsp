...
.file_input label input {
    ...
    overflow:hidden;
        ...
        $(function(){
        	...
        	console.log(cate);
        	var ccate = $("#ccategory").text();
        	console.log(ccate);
        	var idx = -1;
        	var options = $(".ca_d>option");
        	for(var i=0;i<options.length;i++){
        		if(options.eq(i).html() == cate){
        			idx = i;
        		}
        	}
        	...
        	$('.ca_d>option').eq(idx).prop( "selected", true);
   		 	...
   		 	for(var i=0;i<options1.length;i++){
   		 		if(options1.eq(i).html() == ccate){
   		 			idx = i;
   		 		}
   		 	}
   		 	$('.ca_j1>option').eq(idx).prop("selected",true);
   			var options2 =$(".ca_j2>option");
		 	for(var i=0;i<options1.length;i++){
		 		if(options1.eq(i).html() == ccate){
		 			idx = i;
		 		}
		 	}
		 	$('.ca_j2>option').eq(idx).prop("selected",true);
		 	var options3 =$(".ca_j3>option");
   		 	for(var i=0;i<options1.length;i++){
   		 		if(options2.eq(i).html() == ccate){
   		 			idx = i;
   		 		}
   		 	}
   		 	$('.ca_j3>option').eq(idx).prop("selected",true);
   			 var options4 =$(".ca_j4>option");
		 	for(var i=0;i<options1.length;i++){
		 		if(options4.eq(i).html() == ccate){
		 			idx = i;
		 		}
		 	}
		 	$('.ca_j4>option').eq(idx).prop("selected",true);
		 	var options5 =$(".ca_j5>option");
   		 	for(var i=0;i<options5.length;i++){
   		 		if(options5.eq(i).html() == ccate){
   		 			idx = i;
   		 		}
   		 	}
   		 	$('.ca_j5>option').eq(idx).prop("selected",true);
   		 	var options6 =$(".ca_j6>option");
		 	for(var i=0;i<options1.length;i++){
		 		if(options6.eq(i).html() == ccate){
		 			idx = i;
		 		}
		 	}
		 	$('.ca_j6>option').eq(idx).prop("selected",true);
		 	var options7 =$(".ca_j7>option");
   		 	for(var i=0;i<options7.length;i++){
   		 		if(options7.eq(i).html() == ccate){
   		 			idx = i;
   		 		}
   		 	}
   		 	$('.ca_j7>option').eq(idx).prop("selected",true);
   	    	 var options8 =$(".ca_j8>option");
		 	for(var i=0;i<options8.length;i++){
		 		if(options8.eq(i).html() == ccate){
		 			idx = i;
		 		}
		 	}
		 	$('.ca_j8>option').eq(idx).prop("selected",true);
   		 	
   		 	
        	/* if(cate == '레슨'){
        		$('.ca_d>option').eq(1).prop( "selected", true);
        		 $(".ca_j2").show();
        		
        	}
        	if(cate == '홈/리빙'){
        		$('.ca_d>option').eq(2).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '이벤트'){
        		$('.ca_d>option').eq(3).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '비즈니스'){
        		$('.ca_d>option').eq(4).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '디자인/개발'){
        		$('.ca_d>option').eq(5).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '건강/미용'){
        		$('.ca_d>option').eq(6).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '알바'){
        		$('.ca_d>option').eq(7).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
        	if(cate == '기타'){
        		$('.ca_d>option').eq(8).prop( "selected", true);
        		 $(".ca_j2").show();	
        	}
           */
...
<div id="ccategory"  style="display: none;">${fs.fsChildCategory}</div>
            ...
            <section class="resume-section" id="about">
                <div class="resume-section-content">
                    ...
                    <div>
                        <table class="t1">
                            <tr>
                                <td colspan="3" class="t1-1">서비스 등록 <sapn style="color: rgb(196, 189, 255); font-weight: bolder;">tip</sapn></td>
                            </tr>
                            <tr>
                                <td class="t1-2">
                                    이미지 등록<br>
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between mb-5">
                        <div class="flex-grow-1">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="text" name="fsTitle" class="ti1" value="${fs.fsTitle}" >
                            </div>
                            <div class="input-group mb-3">
                                <div class="tit">
                                    카테고리
                                ...
                                <table class="t3">       
                                    <tr>
                                        <td>
                                            ...
                                            <div>
                                            <input type="text" class="ti2" name="fsWorking"  value="${fs.fsWorking}"> 일
                                            </div>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <div>
                                            <input type="text" class="ti2" name="fsPrice"  value="${fs.fsPrice}"> 원
                                            </div>
    ...
    <label>
        썸네일 첨부
        <input type="file" name="file" id="fsPhoto" onchange="javascript:document.getElementById('file_route').value=this.value">
    </label>
     ...
     <!--                             <label>
                                 <input type="file" name="fsPhoto" id="fsPhoto"/>
                                    ...
                                    <div class="select_img"><br> <br><img src="/resources/upload/seller/${fs.fsPhoto}" /></div>
                                    </div>
            ...
            <section class="resume-section" id="education">
                <div class="resume-section-content">
                        ...
                        <table class="t1">
                            <tr>
                                <td colspan="3" class="t1-1"><textarea class="ta1"name="fsContent"> ${fs.fsContent}
                                </textarea></td>
                            </tr>
                            <tr>
                                <td class="t1-2">
...
• 선명한 고해상도의 사진을 올려 구매자들의 시선을 사로잡아보세요.<br>
                    ...
                    <br><br>
                    <textarea id="summernote" class="form-control" rows="5" name="fsDescriptionText" > ${fs.fsDescriptionText }</textarea>
           ...
           <div class="sdr">
                            <input type="submit" value="수정하기" class="btn btn-outline-danger">
                            <a href="javascript:hitory.back(-1)" class="btn btn-outline-danger">돌아가기</a>
                        </div>
